...
class Main {
    ...
    editorActionToggleLabeledBookmark(textEditor) {
        ...
        vscode.window.showInputBox({
            ...
            let targetGroup = new group_1.Group(this.defaultGroupName, this.fallbackColor, this.defaultShape, "", this.decorationFactory);
            if (groupName !== "") {
                targetGroup = this.createGroup(groupName);
            ...
            if (label !== "") {
                ...
                let lineText = textEditor.document.lineAt(lineNumber).text.trim();
                let bookmark = new bookmark_1.Bookmark(fsPath, lineNumber, characterNumber, label, lineText, targetGroup, this.decorationFactory);
            ...
            this.tempDocumentDecorations.delete(fsPath);
            this.tempDocumentBookmarks.delete(fsPath);
            // this.tempGroupBookmarks.delete(targetGroup);
            this.tempGroupBookmarks.delete(this.activeGroup);
            this.saveState();
            this.updateDecorations();
            this.treeViewRefreshCallback();
    ...
    jumpToBookmark(bookmark, preview = false) {
        let editor;
        editor = bookmark.fsPath.endsWith('.ipynb') ? 
        vscode.commands.executeCommand("vscode.openWith", vscode.Uri.file(bookmark.fsPath), 'jupyter-notebook') :
        vscode.window.showTextDocument(vscode.Uri.file(bookmark.fsPath), {preview: preview, preserveFocus: preview});
        editor.then(textEditor => {
            try {
