...
const cb = (textEditor) => {
    if (!cache.left) {
        ...
        cache.leftFn = textEditor.document.fileName.replace(/^.*[\\\/]/, "");
        return;
    }
    if (!cache.right) {
        cache.right = textEditor.document.uri;
        cache.rightFn = textEditor.document.fileName.replace(/^.*[\\\/]/, "");
    }
    if (cache.left && cache.right) {
        var diffTitle = cache.leftFn + "<->" + cache.rightFn;
        vscode.commands.executeCommand("vscode.diff", cache.left, cache.right, diffTitle);
        cache = {};
    }
}
